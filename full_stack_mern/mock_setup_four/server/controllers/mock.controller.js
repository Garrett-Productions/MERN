const MockSchema = require('../models/mock.model');  

module.exports.createMock = (req,res) => {
    MockSchema.create(req.body)
...
module.exports.getOneMock = (req,res) => {
    MockSchema.findOne({_id : req.params.id}) // the .id on the right side of our equal sign needs to match our :id in param, within our route
    .then(mock =>{ res.status(201).json(mock), console.log(res.statusCode)})
    .catch(err => {res.status(400).json(err), console.log(err)});
}